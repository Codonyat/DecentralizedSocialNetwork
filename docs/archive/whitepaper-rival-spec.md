> **ARCHIVED — NON-NORMATIVE.** This is a whitepaper for the rival parallel-session spec — a 21M fixed-supply "constitution," a staked/slashed service registry, and tips-only monetization — that was **not** adopted. The normative design (`docs/00`–`docs/12`) instead uses a 140B supply with a Reward Pool, donation-directed emission with lineage-family weighting, Harberger @handles, bonding, and market-priced indexer fees with no staking. This file distills that archived rival design more fully than any single doc in this directory; it does **not** describe the current spec. A whitepaper for the normative design is future work.

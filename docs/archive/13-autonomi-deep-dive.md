> **ARCHIVED — NON-NORMATIVE.** This document belongs to the same later parallel-session research effort as `12-data-layer-implementation.md` and `14-decisions.md` (post-2026-07-11); it is the forensic deep dive on Autonomi's 1.0 collapse and structural unfitness that justified dropping Autonomi as a storage foundation. **Adopted**: the Autonomi drop itself — the normative design (`docs/02-data-layer.md`, `docs/12-storage-and-anchoring.md`) has no storage-network dependency at all, Autonomi or otherwise, so this document's evidence is preserved as the record of why. **Superseded**: any storage venue this document argues for in Autonomi's place, including a normative Arweave archive tier — the current design treats permanence as replicability, not a protocol-guaranteed archive; endowed archiving is an ordinary out-of-protocol paid service. Internal doc-number references (e.g. "doc 12 §1/§6–8") mean other files from this same parallel session.
